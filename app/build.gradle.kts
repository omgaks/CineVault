...
android {
    ...
    defaultConfig {
        ...
        // sherpa-onnx currently ships only the arm64 native libraries used by
        // the supported CineVault test devices.
        ndk {
...
dependencies {
    ...
    implementation(
        "androidx.media3:" +
            "media3-exoplayer:$media3Version"
    )
    implementation(
        "androidx.media3:" +
            "media3-exoplayer-hls:$media3Version"
    )
    implementation(
        "androidx.media3:" +
            "media3-ui:$media3Version"
    )
    implementation(
        "androidx.media3:" +
            "media3-session:$media3Version"
    )
...
// Configure every JVM test task for JaCoCo instrumentation.
...
// AGP registers the variant test tasks lazily. This live collection avoids
// requesting testDebugUnitTest before AGP has created it.
...
val jacocoTestReport by tasks.registering(
    ...
    // AGP 9 packages compiled debug classes into this runtime classes JAR.
    // The older tmp/kotlin-classes directory is no longer populated.
    classDirectories.setFrom(
        fileTree(
            layout.buildDirectory.dir(
                "intermediates/" +
                    "runtime_library_classes_jar/debug"
            )
        ) {
            include("**/classes.jar")
        },
